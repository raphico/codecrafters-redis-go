...
func HandleRpop(s *session.Session, r *protocol.Request) protocol.Response {
	if len(r.Args) != 1 && len(r.Args) != 2 {
		return protocol.NewErrorResponse("wrong number of arguments for 'rpop' command")
	}
	...
	// Reverse to match Redis RPOP order
